...
function Bot() {
	...
	self.Client.on( "user", function(friend) {
	    var steamID = friend.steamid_source;
	    var oldFriend = self.Client.users[steamID];

	    if(oldFriend.player_name != friend.player_name) {
	        self.emit( "UserChangedName", oldFriend.player_name, friend.player_name, steamID );
	    }
	    else if(oldFriend.game_name != friend.game_name) {
	        self.emit( "UserStartedGame", friend.game_name, steamID );
	    }
	    else if(oldFriend.game_server_ip != friend.game_server_ip) {
	        self.emit( "UserJoinedServer", friend.game_server_ip, friend.game_server_port, steamID );
	    }
	} );
